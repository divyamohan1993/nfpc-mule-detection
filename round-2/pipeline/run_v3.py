"""
NFPC Phase 2 — Pipeline Orchestrator (V3)

Uses models_v3 for improved training:
  - No AV debiasing
  - Frequency encoding (no target leakage)
  - Rank averaging ensemble
  - Multi-seed stability
  - Raw probabilities (no isotonic calibration for AUC)

Re-uses existing features and label cleaning from V2.
"""
import sys
import time
import argparse
...
def run_pipeline(skip_optuna=False, force_stage=None):
    ...
    log.info("V3 Pipeline complete in %.1f minutes", elapsed / 60)
    log.info("Output: %s", OUTPUT_DIR / "submission_v3.csv")

    return submission


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NFPC Phase 2 Pipeline V3")
