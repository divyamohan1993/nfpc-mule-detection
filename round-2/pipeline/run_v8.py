"""
NFPC Phase 2 — Pipeline Orchestrator (V8)

Uses models_v8: V3 base + surgical branch_code drop (RH7 fix).
Keeps branch structural features, drops only branch_code_freq/count.
"""
import sys
import time
import argparse
...
def run_pipeline(skip_optuna=False, force_stage=None):
    """Run V8 pipeline — re-uses features, re-trains models."""
    t0 = time.time()
    ...
    # ── Stage 3: Model Training (V8 — always re-run) ──
    _clear_stage("models_v8")

    log.info("══════ STAGE 3: Model Training (V8) ══════")
    from models_v8 import train_and_predict
    results = train_and_predict(
        features,
        sample_weights=sample_weights,
        skip_optuna=skip_optuna,
    )
    _mark_done("models_v8")
    ...
    # Save final CSV
    submission.to_csv(OUTPUT_DIR / "submission_v8.csv", index=False)
    ...
    log.info("V8 Pipeline complete in %.1f minutes", elapsed / 60)
    log.info("Output: %s", OUTPUT_DIR / "submission_v8.csv")

    return submission


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NFPC Phase 2 Pipeline V8")
