"""
NFPC Phase 2 — Pipeline Orchestrator (V6)

Uses models_v6: V3 base + pseudo-labeling for training augmentation.
...
def run_pipeline(skip_optuna=False, force_stage=None):
    ...
    # Save final CSV
    submission.to_csv(OUTPUT_DIR / "submission_v6.csv", index=False)
    ...
    log.info("V6 Pipeline complete in %.1f minutes", elapsed / 60)
    log.info("Output: %s", OUTPUT_DIR / "submission_v6.csv")

    return submission


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NFPC Phase 2 Pipeline V6")
