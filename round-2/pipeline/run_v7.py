"""
NFPC Phase 2 — Pipeline Orchestrator (V7)

Uses models_v7: V3 base minus branch features (Red Herring #7 avoidance).
"""
import sys
import time
import argparse
...
def run_pipeline(skip_optuna=False, force_stage=None):
    ...
    # Save final CSV
    submission.to_csv(OUTPUT_DIR / "submission_v7.csv", index=False)
    ...
    log.info("V7 Pipeline complete in %.1f minutes", elapsed / 60)
    log.info("Output: %s", OUTPUT_DIR / "submission_v7.csv")

    return submission


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NFPC Phase 2 Pipeline V7")
