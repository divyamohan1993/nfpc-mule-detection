"""
NFPC Phase 2 — Model Training & Inference (V8)

Key changes from V3:
  1. All V3 improvements retained
  2. Surgical RH7 fix: drop ONLY branch_code frequency/count encodings
     (these let the model learn "which specific branch" = RH7 pathway)
     Keep branch_employee_count, branch_n_accounts, branch_type, etc. (real signal)
...
def train_and_predict(features, sample_weights=None, skip_optuna=False):
    ...
    # V8: Surgical RH7 fix — drop ONLY branch_code frequency/count encodings
    # These let the model learn "which specific branch" = RH7 leakage pathway
    # Keep branch_employee_count, branch_n_accounts, branch_type, etc. (real signal)
    rh7_drop = [c for c in X_train.columns if c.lower() in [
        "branch_code_freq", "branch_code_count",
    ]]
    if rh7_drop:
        log.info("V8: Dropping %d branch-code ID features to avoid RH7: %s", len(rh7_drop), rh7_drop)
    ...
    log.info("Feature count: %d (V8: surgical branch-code drop)", len(common_cols))

    # ── Step 2: Optuna HPO ──
    saved_params_path = MODELS_DIR / "best_params_v8.pkl"
    if not skip_optuna:
    ...
    try:
        ...
        shap_imp.to_csv(OUTPUT_DIR / "shap_importance_v8.csv")
    ...
    # ── Save outputs ──
    joblib.dump(meta, MODELS_DIR / "meta_learner_v8.pkl")
    ...
    oof_df.to_csv(OUTPUT_DIR / "oof_predictions_v8.csv", index=False)
