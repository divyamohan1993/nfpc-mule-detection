"""
NFPC Phase 2 — Model Training & Inference (V7)

Key changes from V3:
  1. All V3 improvements retained
  2. DROP branch-related features to avoid Red Herring #7
     (accounts flagged by their own branch — spurious pattern)
  3. Uses V2 params (best known public score)
"""
import numpy as np
import pandas as pd
...
def train_and_predict(features, sample_weights=None, skip_optuna=False):
    ...
    # V7: Drop branch-related features to avoid Red Herring #7
    # RH7 = "accounts flagged by their own branch" — branch features capture
    # a spurious same-branch pattern that doesn't generalize to private test
    rh7_drop = [c for c in X_train.columns if any(
        pat in c.lower() for pat in [
            "branch_code", "branch_employee", "branch_n_account",
            "accounts_per_employee", "branch_type", "rural_branch",
        ]
    )]
    if rh7_drop:
        log.info("V7: Dropping %d branch features to avoid RH7: %s", len(rh7_drop), rh7_drop)
        X_train = X_train.drop(columns=rh7_drop, errors="ignore")
        X_test = X_test.drop(columns=rh7_drop, errors="ignore")
    ...
    log.info("Feature count: %d (V7: no branch features)", len(common_cols))

    # ── Step 2: Optuna HPO ──
    saved_params_path = MODELS_DIR / "best_params_v7.pkl"
    if not skip_optuna:
        log.info("Starting Optuna hyperparameter optimization (V7)")
        ...
        log.info("Loading Optuna V7 params")
    ...
    try:
        ...
        shap_imp.to_csv(OUTPUT_DIR / "shap_importance_v7.csv")
    ...
    # ── Save outputs ──
    joblib.dump(meta, MODELS_DIR / "meta_learner_v7.pkl")
    ...
    oof_df.to_csv(OUTPUT_DIR / "oof_predictions_v7.csv", index=False)
    ...
    log.info("V7 Training complete. Best ensemble AUC: %.5f (%s)", best_auc, best_method)
